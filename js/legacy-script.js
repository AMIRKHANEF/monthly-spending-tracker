class SpendingTracker {
  ...
  showMonthExpensesModal() {
    ...
    document.getElementById("monthModalTitle").textContent = `هزینه‌های ${monthName} ${jy}`
    const monthExpensesListContainer = document.getElementById("monthExpensesList")
      ...
      sortedDates.forEach((dateKey) => {
        ...
        daySection.innerHTML = `
          <h4 style="margin-top: 15px; margin-bottom: 10px; color: var(--theme-accent); font-weight: 600;">
            ...
            <span style="float: left; color: var(--theme-text);">${this.formatAmount(dayTotal)} ت</span>
          </h4>
        ...
        dayExpenses.forEach((expense) => {
          ...
          `
          expenseElement.addEventListener("click", (e) => this.toggleExpenseSelection(e.currentTarget))
    ...
    document.getElementById("monthTotal").textContent = `${this.formatAmount(monthTotal)} تومان`
  ...
  toggleExpenseSelection(expenseElement) {
    const amount = Number.parseFloat(expenseElement.getAttribute("data-amount"))
  ...
  updateSelectedTotalDisplay() {
    document.getElementById("selectedExpensesTotal").textContent =
      `${this.formatAmount(this.selectedExpensesTotal)} تومان`
  }
  ...
  bindEvents() {
    ...
    document.getElementById("closeMonthExpensesModal").addEventListener("click", () => this.closeMonthExpensesModal())
    document.getElementById("viewMonthExpenses").addEventListener("click", () => this.showMonthExpensesModal())
    ...
    document.getElementById("monthExpensesModal").addEventListener("click", (e) => {
      if (e.target.id === "monthExpensesModal") this.closeMonthExpensesModal()
    })
  ...
  bindModalEvents(persianDateKey) {
    ...
    editButtons.forEach((btn) => {
      ...
      const newBtn = document.querySelector(`[data-expense-id="${btn.dataset.expenseId}"].edit-expense-btn`)
  ...
  showConfirmModal(onConfirm, expenseData) {
    ...
    expensePreview.innerHTML = `
      ...
      <div class="expense-amount">${this.formatAmount(expenseData.amount)} تومان</div>
    `
